...
Only upload **your first-party code** (code written by the user, not framework-generated) — not Drupal framework files. The entire application lives at `/var/www/html` inside the container, but the vast majority is Drupal core and third-party packages that are not your code.
...
├── modules/
│   ├── contrib/                 ← SKIP  Downloaded Drupal modules (3rd party)
│   └── custom/                  ← INCLUDE  First-party code (user-written)
│       ├── vulnerable_api/
│       ├── vulnerable_auth/
│       └── vulnerable_upload/
├── themes/
│   ├── contrib/                 ← SKIP  Downloaded themes (3rd party)
│   └── custom/                  ← INCLUDE  First-party code (user-written)
│       └── vulnerable_theme/
├── sites/
│   └── default/
│       ├── settings.php         ← SKIP  Generated at runtime by Drush
│       ├── docker-settings.php  ← INCLUDE  First-party config (user-written intentional vulnerabilities)
│       ├── settings.local.php   ← INCLUDE  First-party config (user-written intentional vulnerabilities)
│       └── files/               ← SKIP  User-uploaded files
├── index.php                    ← SKIP  Drupal scaffolded entry point (framework-generated)
├── autoload.php                 ← SKIP  Drupal scaffolded (framework-generated)
└── ...                          ← SKIP  All other root files are Drupal scaffolding (framework-generated)
```

**Summary — only these paths contain first-party code (user-written, not framework-generated):**

| Path | What it is |
|------|-----------|
| `modules/custom/vulnerable_api/` | **First-party code**: REST API with SQL injection, CORS, hardcoded credentials |
| `modules/custom/vulnerable_auth/` | **First-party code**: Login with SQL injection, weak hashing, session fixation |
| `modules/custom/vulnerable_upload/` | **First-party code**: File upload with path traversal, XXE, insecure permissions |
| `themes/custom/vulnerable_theme/` | **First-party code**: Templates with XSS and missing CSRF tokens |
| `sites/default/docker-settings.php` | **First-party config**: Intentional config vulnerabilities (debug mode, weak crypto, etc.) |
| `sites/default/settings.local.php` | **First-party config**: Additional intentional config vulnerabilities |

### Preparing the Scan Package

Run this from the project root to create a clean ZIP containing only your first-party code (user-written, not framework-generated):
...
**Note:** This ZIP contains only first-party code written by users, excluding all framework-generated and third-party code.
...
This means **the same packaging rule applies to any Drupal 10 project**: include only first-party code from `modules/custom/`, `themes/custom/`, and any custom settings files. Skip all framework-generated and third-party code.
...
Drupal 7 also does not use Composer or the `src/Controller/` object-oriented pattern. Our project is **Drupal 10** — the modern standard. Remember: always include only first-party code (user-written), not framework-generated code.
...
Regardless of these differences, **your first-party code is always in `modules/custom/` and `themes/custom/`** — that never changes in Drupal 8/9/10. This first-party code is written by users and is distinct from framework-generated scaffolding.
...
Every Drupal module and theme (both framework-generated and first-party user code) has one with this exact format:
```yaml
type: module
core_version_requirement: ^10.0
```
The `core_version_requirement` field is Drupal-specific. First-party code modules will have `.info.yml` files in `modules/custom/`.

**3. `sites/default/settings.php` with `$databases` array**

The `sites/` directory with this exact PHP array syntax is unique to Drupal. Most of `settings.php` is framework-generated, but first-party code often creates custom settings files like `docker-settings.php` or `settings.local.php` in the same directory:
```php
...
This exact trio at the root level is Drupal's signature layout. First-party code lives in the `custom/` subdirectories: `modules/custom/`, `themes/custom/`, and `profiles/custom/`.

**5. `autoload.php` generated by `drupal-scaffold`**

The comment inside indicates this is framework-generated code, not first-party user code:
```php
// This file was generated by drupal-scaffold.
```
This is a clear indicator of framework scaffolding that should be excluded from security scans.
...
```
The `composer.json` file itself is first-party code (written by developers), but the packages it requires are third-party dependencies.
